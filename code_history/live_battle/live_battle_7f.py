...
# --- GEOMETRIC HEURISTICS (The "Rock" Fix) ---
def is_fist(landmarks):
    """
    Returns True if the hand is geometrically closed (Rock).
    Logic: If finger tips are closer to wrist than their knuckles (MCP).
    """
    # Landmarks: 0=Wrist, 9=MiddleMCP, 12=MiddleTip, 13=RingMCP, 16=RingTip, 17=PinkyMCP, 20=PinkyTip
    ...
    # Check Middle, Ring, Pinky
    # If Tip is closer to Wrist than MCP is, the finger is bent/closed.
    middle_closed = dist(12, 0) < dist(9, 0)
    ring_closed   = dist(16, 0) < dist(13, 0)
    pinky_closed  = dist(20, 0) < dist(17, 0)
    
    # If 3 main fingers are closed, it's a Rock.
...
def run_game():
    ...
    last_features = None 
    last_landmarks = None # Store skeleton for geometric check
    ...
    while True:
        ...
        if results.multi_hand_landmarks:
            ...
            last_landmarks = hand_landmarks # Save for logic check
        ...
        if state == "IDLE":
            ...
            # SLOWER COUNTDOWN (More suspense)
            if elapsed < 1.5: 
                ui_text = "1..."
            elif elapsed < 3.0: 
                ui_text = "2..."
            elif elapsed < 3.8: # EARLY TRIGGER! (Before "GO")
                ui_text = "3..." 
                ui_color = (0, 255, 255) 
            else:
                # --- PREDICT NOW (Frame 44/64 approx) ---
                state = "PREDICT"
            ...
            if len(feature_buffer) == SEQ_LEN:
                ...
                with torch.no_grad():
                    ...
                    # --- THE "ROCK" OVERRIDE ---
                    # If model says Paper/Scissors, but hand is physically closed -> ROCK
                    if last_landmarks and is_fist(last_landmarks):
                        if pred_idx != 0: # If not Rock
                            print(f"Model guessed {CLASS_MAP[pred_idx]} but hand is CLOSED. Overriding to Rock.")
                            pred_idx = 0 # Force Rock
                            conf_val = 1.0 # Force Confidence
                    
                    user_move = CLASS_MAP[pred_idx]
                ...
                ui_text = "Wait..." # Should be rare

        elif state == "RESULT":
            ui_text = final_result_text
            ui_color = (0, 255, 0) # Win!
