...
# --- SPLIT ARCHITECTURE ---
# 1. The Eye (ResNet) - Runs every frame
...
# 2. The Brain (TCN) - Runs only on "GO!"
class GestureTCNHead(nn.Module):
    ...
    def forward(self, features):
        # features: (Batch, Seq, Dim) -> TCN: (Batch, Dim, Seq)
        features = features.permute(0, 2, 1)
        tcn_out = self.tcn(features)
        last_out = tcn_out[:, :, -1]
        return self.classifier(last_out)

# Wrapper for loading weights (Matches your training structure)
...
def run_game():
    ...
    # BUFFERS
    # Rolling buffer of FEATURES (not images).
    # This means the heavy processing is already done when we need it.
    ...
    while True:
        ...
        current_features = None # (128,) vector
        ...
        if state == "IDLE":
            ...
            if len(feature_buffer) == SEQ_LEN:
                # ZERO LAG PREDICTION
                # We just take the buffer of numbers we already calculated
                ...
                with torch.no_grad():
                    ...
                    print(f"Prediction: {user_move} ({result_confidence:.2f})")
                    
                    state = "RESULT"
                    start_time = time.time()
            else:
                ui_text = "Buffering..."
                # Fill buffer with last frame if needed
                while len(feature_buffer) < SEQ_LEN and len(feature_buffer) > 0:
                    feature_buffer.append(feature_buffer[-1])

        elif state == "RESULT":
            ui_text = final_result_text
            ui_color = (0, 255, 0) if result_confidence > 0.8 else (200, 200, 0)
            if time.time() - start_time > 4.0: 
                state = "IDLE"
                feature_buffer.clear() # Reset for next round
        ...
        # Buffer Bar
        bar_len = int(w_img * len(feature_buffer)/SEQ_LEN)
        cv2.line(frame, (0, h_img-5), (bar_len, h_img-5), (0, 255, 0), 4)
        ...
        if key == ord(' ') and state == "IDLE":
            state = "COUNTDOWN"
            # We DON'T clear buffer here. We let the "1...2..." phase fill it with fresh data.
            start_time = time.time()
