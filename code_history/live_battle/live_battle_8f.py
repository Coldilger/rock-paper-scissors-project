...
def is_fist(landmarks):
    """
    Rock Check: Are fingers curled in?
    Condition: Tip is CLOSER to Wrist (0) than Knuckle (MCP) is.
    """
    wrist = 0
    # Check Middle (12), Ring (16), Pinky (20) tips vs their MCPs (9, 13, 17)
...
def is_paper(landmarks):
    """
    Paper Check: Are 4 fingers open? (Thumb ignored)
    Condition: Tip is FURTHER from Wrist (0) than Knuckle (MCP) is.
    """
    wrist = 0
...
def run_game():
    ...
    while True:
        ...
        if state == "IDLE":
            ...
            # SLOWER TIMING: 2.0s per step
            if elapsed < 2.0: 
                ui_text = "1..."
            elif elapsed < 4.0: 
                ui_text = "2..."
            
            # TRIGGER at 3.8s (200ms before '4.0s' which would be GO)
            if elapsed > 3.8: 
                state = "PREDICT" 
                ...
                with torch.no_grad():
                    ...
                    if last_landmarks:
                        ...
                        if is_fist(last_landmarks):
                            if pred_idx != 0: 
                                # print("Override: Force Rock")
                                pred_idx = 0 
                                conf_val = 1.0
                        
                        # 2. Check Paper (New!)
                        elif is_paper(last_landmarks):
                            if pred_idx != 1:
                                # print("Override: Force Paper")
                                pred_idx = 1
                                conf_val = 1.0
